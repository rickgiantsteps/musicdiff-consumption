...
for k in datasets:
    ...
    for i, file in enumerate(tqdm(filenames, desc=f"Processing MagnetMedium dataset {k}", unit="item")):

        if os.path.exists(f'audios/genaudios/Magnet/Medium/{k.split("_")[0]}/MagnetMedium-'+file):               
            ...
            tracker = EmissionsTracker(project_name=f"MagnetMedium_{k.split('_')[0]}-{file.split('.')[0]}", tracking_mode="process",
                gpu_ids = "0",
                output_dir="../../results/quality_metrics/Emissions/Magnet",
                output_file=f"MagnetMedium_{k.split('_')[0]}.csv", allow_multiple_runs=True)
            ...
            for idx, one_wav in enumerate(wav):
                audio_write(f"audios/genaudios/Magnet/Medium/{k.split('_')[0]}/MagnetMedium-{file}",
