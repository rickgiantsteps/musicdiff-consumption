...
for k in datasets:
    ...
    for i, file in enumerate(tqdm(filenames, desc=f"Processing MusicGenLarge dataset {k}", unit="item")):

        if os.path.exists(f'audios/genaudios/MusicGen/Large/{k.split("_")[0]}/MusicGenLarge-'+file):               
            ...
            tracker = EmissionsTracker(project_name=f"MusicGenLarge_{k.split('_')[0]}-{file.split('.')[0]}", tracking_mode="process",
                gpu_ids = "0",
                output_dir="../../results/quality_metrics/Emissions/MusicGen",
                output_file=f"MusicGenLarge_{k.split('_')[0]}.csv", allow_multiple_runs=True)
            ...
            scipy.io.wavfile.write(f"audios/genaudios/MusicGen/Large/{k.split('_')[0]}/MusicGenLarge-{file}",
